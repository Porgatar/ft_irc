...
void    Irc::invite(User &actual)
{
    ...
    //si ce user est sur le channel ou il souhaite inviter quelquun
    else if (getChannelIteratorByName(_args[2])->isIn(USER, actual.getNickname()) == false)
        return;
    else if (getChannelIteratorByName(_args[2])->isIn(USER, _args[1]) == true)
        return;
    else {
        ...
        getChannelIteratorByName(_args[2])->addTo(INVITE, *it);
    }
}